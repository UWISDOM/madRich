...
#'
#' @param df result of enrichment function. Should contain a column of pathway names ("pathway"), p-values ("pval"), gene set collections ("gs_cat"), and gene set subcollections where applicable ("gs_subcollection"). If filtering by FDR, k/K, or NES, these values must be included as "FDR", "k/K", or "NES", respectively.
...
#Errors
  ...
  if(is.null(db)){ # if database is not provided
    ...
    for(cat in collections){
      database <- msigdbr::msigdbr(db_species = db_species, collection = cat) # get msigdb reference
      
      # Fix C2 subcollection names
      if(cat == "C2"){
        database <- database %>% # subcollections in C2 is formatted bad. separate "CP" from "KEGG", etc.
          tidyr::separate_wider_delim(gs_subcollection, ":", 
                                      names = c("gs_subcollection_format", "x"), 
      ...
      #Deal with multiple subcollection per cat
      subcollections <- subcollections[names(subcollections)==cat]
      if(length(subcollections)>0){
        # filter to subcollection when applicable
        database <- database %>% 
          dplyr::filter(gs_subcollection_format %in% subcollections)
      }
  ...
  if(length(unique(db_format$sign)) > 1){
    ...
    for(s in unique(db_format$sign)){# separates if positive and negative signs
      ...
      # output df of n clusters vs cut height
